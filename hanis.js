...
var HAniS = new function() {
  ...
  numFrames, backFiles, numOverlays, overlayFiles, 
  overlayTop, overlayLabels,overlayOrder,
  ...
  begFrame, isRunning, isLooping, wasLooping, curFrame, direction, isRocking,
  fetchImages, needSizes=true, chkImageSize, isCtrlKey,
  configValues, dwell, minDwell, maxDwell,
  ...
  mark, isMark, markMode, markPrompts, markFont, 
  ...
  wasZooming, enableZooming, zoomFactorMax, isDragging, cycleZoom,
  doTransparency=false, transRed, transGreen, transBlue,
  useTransparencyList, transparencyList,
  ...
  xText, yText, wText, hText, winFirst=true, initCall,
  ...
  this.setup = function(confn,divnam, initCallback) {
  ...
  function parseConfig (txt) {
    ...
    if (overlayLabels != null && preserveIndex != null) {
      ...
      for (i=0; i<preserveIndex.length; i++) {
        if (preserveIndex[i]) {
          ...
          if (b != null) {
            preservePoints[i][4] = parseInt(b[p/2-2],10);
            preservePoints[i][5] = parseInt(b[p/2-1],10);
          } else {
            preservePoints[i][4] = preservePoints[i][0];
            preservePoints[i][5] = preservePoints[i][1];
          }
        }
      }
    }
    ...
    if (cv != null) {
      hideBottomDef = parseInt(cv.trim(),10);
    }
    ...
    if (cv != null) {
      hideTopDef = parseInt(cv.trim(),10);
    }
  ...
  function next2() {
    ...
    if (cv != null) {
      ...
      for (var k=0; k<spritefn.length; k++) {
        ...
        spriteImages[k].zoomme = (spritefn[k].indexOf("/zoom") > 0);
        spriteImages[k].anim = (spritefn[k].indexOf("/animate") > 0);
        var ss = spritefn[k].indexOf("/");
    ...
    for (i=0; i<numFrames; i++){
      ...
      backImages[i].onload = function() {
        ...
        if (numHoverzones > 0 && !gotHoverzones) {
          ...
          if (zoomXFactor == 1.0) doHoverzones = true;
          gotHoverzones = true;
        }
              ...
              for (j=0; j<numOverlays; j++) {
                ...
                overlayImages[k][j].onload = function() {
                  ...
                  if (this.gotit && overlayTop[this.overlayNum]) {
                    ...
                    ctxtp.drawImage(this,sliceCoords[0], sliceCoords[1], sliceCoords[2],sliceCoords[3],0,0,imgWidth, imgHeight);
                    overlayImages[f][this.overlayNum] = null;
                    overlayImages[f][this.overlayNum] = tpcan;
                    overlayImages[f][this.overlayNum].gotit = true;
  ...
  function configButton(button, name, defOn, defOff, tip) {
  ...
  function doMakeControls(control, isTop) { 
    ...
    for (i=0; i<c.length; i++) {
      ...
      if (cstr == "startstop") {
        ...
        if (useCN) startstop.className = "startstop start";
      ...
      if (cstr == "looprock") {
        ...
        if (useCN) looprock.className = "looprock rock";
      ...
      if (cstr == "extrap") {
        extrap = make("button");
        if (useCN) extrap.className = "extrap";
      ...
      if (beginsWith(cstr,"location")) {
        ...
        if (useCN) location.className = "location";
      ...
      if (cstr == "distance") {
        ...
        if (useCN) distance.className = "distance";
      ...
      if (cstr == "probe") {
        probe = make("button");
        if (useCN) probe.className = "probe";
      ...
      if (cstr == "mark") {
        mark = make("button");
        if (useCN) mark.className = "mark";
        configButton(mark, "mark", "Mark", "Stop Mark", mytip);
        drawPrompts = true;
      ...
      if (cstr == "refresh") {
        refresh = make("button");
        if (useCN) refresh.className = "refresh";
      ...
      if (cstr == "autorefresh" || cstr == 'autorefresh/off') {
        ...
        if (useCN) autotoggle.className = "autorefresh";
      ...
      if (cstr == "backward") {
        ...
        configButton(backward, "backward", "<", "<", mytip);
      ...
      if (cstr == "forward") {
        ...
        configButton(forward, "forward", ">", ">", mytip);
      ...
      if (cstr == "first") {
        first = make("button");
        if (useCN) first.className = "firstlast first";
        configButton(first, "first", "|<", "|<", mytip);
      ...
      if (cstr == "last") {
        last = make("button");
        if (useCN) last.className = "firstlast last";
        configButton(last, "last", ">|", ">|", mytip);
      ...
      if (cstr == "zoom") {
        zoom = make("button");
        if (useCN) zoom.className = "zoom off";
      ...
      if (cstr == "annotate") {
        ...
        if (useCN) annotate.className = "annotate";
        configButton(annotate, "annotate", "Annotate", "Finished", mytip);
        // create "divannot", but hide it
        divannot = make("div");
        divannot.setAttribute("style","position:absolute;height:120px;width:180px;top:20px;left:20px;z-index:15;visibility:hidden;background-color:lightgray;border-width:3px;border-style:solid;border-color:magenta;border-radius:25px;padding:10px;font-family:Arial;font-size:16px;color:black;");
        ...
        // these 3 event handlers are for moving the div around
        ...
        ancol = makeSelect( ["Color","Black","White","Red","Green","Blue","Cyan","Magenta"], ["black","black","white","red","green","blue","cyan","magenta"], function(e) {
        });
      ...
      if (cstr == "saveall") {
        saveall = make("button");
        if (useCN) saveall.className = "saveall";
        configButton(saveall, "saveall", "SaveAll", "SaveAll", mytip);
      ...
      if (cstr == "capture") {
        capture = make("button");
        if (useCN) capture.className = "capture";
        configButton(capture, "capture", "Capture", "Capture", mytip);
        ...
        if (markMode > 0) {
          if (yLoc/canYScale < markHbeg) {
            if(xLoc/canXScale > markXbeg) {
              ...
              markwin.document.write(marsav[0].toFixed(4)+","+marsav[1].toFixed(4)+"<br>");
              return;
            }
          }  
        }
  ...
  function doAutoEnhance(img, ox, oy, ow, oh, tn) {
  ...
  function drawLines() {
    ...
    if (isMark) {
      ...
      if (markPoints.length>4) ctxd.lineTo(markPoints[0]/canXScale, markPoints[1]/canYScale);
      ctxd.stroke();
      for (mps=0; mps<markPoints.length; mps=mps+2) {
        ctxd.beginPath();
          ...
          if (doBaseProbe) {
            ...
            for (mk=0; mk<numOverlays; mk++) {
              ...
              if (overlayImages[curFrame][k].gotit && 
                ...
                rgb = ctx1.getImageData(0,0,1,1).data;
                tn = overlayProbe[k];
                probeScale(( k == overlayEnhNum) || tabEnh);
  ...
  function probeScale(pt) {
    ...
    drawText(probeBox, xScreen-10, yScreen, pValue, 1);
  }
        ...
        if (preserveIndex != null) {
          ...
          for (var ii=0; ii<numOverlays; ii++) {
            ...
            if (overlayImages[curFrame][i].gotit && 
                ...
                preservePoints[i][4]/canXScale, preservePoints[i][5]/canYScale, 
                preservePoints[i][2]/canXScale, preservePoints[i][3]/canYScale);
            }
          }
        }
